---
title: "OpenCyto Tutorial<img src='resources/Cyto_logo_title.png', width='250px'><br><font size=6>Robust and Reproducible Automated Gating
...
<footer>www.isac-net.org</footer>
...
```{r example_Perforin,echo=FALSE,eval=TRUE,fig.cap='Automated and Manual Gating of Perforin/CD8',cache=TRUE,fig.align='center',fig.width=7.5,fig.height=3.5,out.width=500}
...
```{r example_TNFa,echo=FALSE,eval=TRUE,cache=TRUE,fig.cap="Automated and manual gating of TFNa",fig.align='center'}
...
* Gating of HIV vaccine clinical trial ICS data at the HVTN
    * Re-analysis of RV144, HVTN080, HVTN078,...
* Gating of ICS data from tuberculosis vs. healthy subjects.
* Gating of CyTOF data
...
* Easily pushed into downstream statstical analysis methods like MIMOSA (PMC3862207) or COMPASS (http://rglab.github.org/COMPASS/)   
<emph>We're usually pretty friendly and can help you get started</emph>

## Summary {.smaller}

* OpenCyto is a <emph>framework</emph> for constructing standardized analysis pipelines for standardized assays.
* Plugins allow any gating algorithm to be used for gating any cell pouplation (even high-dimensional gating, i.e. not just 1-D or 2-D) 
    * e.g  <emph>flowDensity</emph> is supported.
* If the data coming off of an instrument are consistent, then an <emph>OpenCyto gating template can be reused</emph> for any data set using the same panel.
     * Work is done up front to set up the template.
     * Fully reproducible and <emph>*objective, data-driven gating*</emph>.
* Framework handles <emph>data manipulation</emph>.
* With adequate SOP's in place, the analysis code can also be re-used.
    * Code reads data, performs preprocessing, compensation, transformation, and gating using a template.
    * Generates plots, writes out statistics, can perform analysis and QC.
    * All this can be standardized with adequate SOPs (standard naming conventions, good use of Keywords, and so forth.)
